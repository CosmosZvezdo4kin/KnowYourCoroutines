...
{
    public string Command => "running";

    public string[] Aliases => Array.Empty<string>();

    public string Description => "Get list of all running coroutines";
    ...
    public bool Execute(ArraySegment<string> arguments, ICommandSender sender, out string response)
    {
        ...
        var coroutinesList = Timing._instance._indexToHandle.Where(x => x.Value.IsRunning).OrderByDescending(x => x.Value._id).ToList();

        var result = string.Join("\n", coroutinesList.Select(x => $"<color=red>{x.Value._id}</color> {x.Value}"));
        
        response = $"Running coroutines count: <b>{coroutinesList.Count}</b>\n{result}";
        return true;
    }
}